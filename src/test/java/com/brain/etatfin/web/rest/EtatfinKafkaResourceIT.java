...
import com.brain.etatfin.config.KafkaProperties;
...
class EtatfinKafkaResourceIT {
    ...
    void setup() {
        ...
        EtatfinKafkaResource kafkaResource = new EtatfinKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/etatfin-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/etatfin-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
